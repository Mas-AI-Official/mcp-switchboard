@echo off
REM ============================================================================
REM  Switchboard - one-click launcher (Windows)
REM
...
if errorlevel 1 (
  echo.
  echo   [Switchboard] Node.js was not found on your PATH.
...
if not exist "node_modules\" (
  echo   [Switchboard] Installing dependencies ^(first run, ~1 minute^)...
  call npm install
  if errorlevel 1 (
    echo   [Switchboard] npm install failed - see the messages above.
    pause
    exit /b 1
  )
)
...
if not exist "dist\cli.js" (
  echo   [Switchboard] Building...
  call npm run build
  if errorlevel 1 (
    echo   [Switchboard] build failed - see the messages above.
    pause
    exit /b 1
  )
)
...
if not exist "switchboard.config.yaml" (
  echo   [Switchboard] Creating a starter switchboard.config.yaml...
...
echo.
echo   Switchboard is starting on http://127.0.0.1:%PORT%
