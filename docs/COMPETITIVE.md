# MCP Switchboard — Competitive Landscape
...
## The one axis that separates MCP Switchboard from the field
...
tokens transit their infrastructure. MCP Switchboard keeps them in a **local AES-256-GCM vault on your
...
| Player | OSS vs hosted | Where creds / OAuth tokens live | MCP support | License | #1 difference vs. local-first self-hosted MCP Switchboard |
|---|---|---|---|---|---|
...
| **ACI.dev (Aipolabs)** | OSS | Self-host stores creds **locally**, but the self-host catalog is limited; the full 600+ is **cloud-only** | Unified MCP server + SDK | Apache-2.0 | Closest peer — but its full catalog is gated to hosted; MCP Switchboard ships full BYO with no hosted dependency |
...
credential custody (theirs) and self-hostability (partial or paid). MCP Switchboard's lane — full BYO,
