@echo off
REM ============================================================================
REM  Switchboard - stop the running gateway (Windows)
REM
...
echo   [Switchboard] Stopping anything listening on port %PORT% ...
...
if not defined FOUND (
  echo   [Switchboard] nothing was listening on port %PORT% - already stopped.
) else if defined KILLED (
  echo   [Switchboard] done.
) else (
  echo   [Switchboard] found a process on port %PORT% but could not stop it.
  echo   [Switchboard] try again from an Administrator command prompt.
)
exit /b 0
...
if not errorlevel 1 (
  echo   [Switchboard] stopped PID %_PID%.
  set "KILLED=1"
  goto :eof
)
...
if not errorlevel 1 (
  echo   [Switchboard] stopped PID %_PID% ^(via PowerShell^).
  set "KILLED=1"
)
goto :eof
