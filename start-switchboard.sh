#!/usr/bin/env bash
# ============================================================================
#  Switchboard - one-command launcher (macOS / Linux)
#
...
command -v node >/dev/null 2>&1 || {
  echo "[Switchboard] Node 18.18+ is required - install it from https://nodejs.org" >&2
  exit 1
}

[ -d node_modules ] || { echo "[Switchboard] Installing dependencies (first run)..."; npm install; }
[ -f dist/cli.js ]  || { echo "[Switchboard] Building...";                              npm run build; }
[ -f switchboard.config.yaml ] || { echo "[Switchboard] Creating a starter config..."; node dist/cli.js init; }
...
echo
echo "  Switchboard is starting on ${URL}"
